  ...
  return (
    <div className="p-4 space-y-3 max-w-[1400px] mx-auto">
      <nav className="text-xs text-slate-500 flex flex-wrap gap-1 items-center" aria-label="Breadcrumb">
        <Link to="/dashboard" className="hover:text-gov-navy">Dashboard</Link><span className="text-slate-700">/</span>
        <Link to="/map" className="hover:text-gov-navy">Map</Link>
      ...
      <div className="space-y-3">
        <div className="panel p-2.5 flex flex-wrap gap-x-3 gap-y-2 items-center text-sm">
          ...
          <label className="text-xs flex gap-1.5 items-center text-slate-600"><input type="checkbox" className="accent-[#1A3A6B]" checked={exploded} onChange={e => setExploded(e.target.checked)} />Exploded view</label>
          <label className="text-xs flex gap-1.5 items-center text-slate-600"><input type="checkbox" className="accent-[#1A3A6B]" checked={underground} onChange={e => setUnderground(e.target.checked)} />Underground mode</label>
      ...
      <div className="space-y-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {floor && <div className="grid grid-cols-4 gap-1 mt-2 max-h-40 overflow-auto scrollthin">
                {floor.units?.slice(0, 24).map((u: any) => (
                  <button key={u.ulpin} onClick={() => setUnitId(u.ulpin)} className={`text-[11px] border rounded px-1 py-1 ${unitId === u.ulpin ? 'bg-emerald-500 text-white font-bold border-emerald-400' : 'border-slate-200 text-slate-600 hover:bg-slate-100'}`}>{u.unit}</button>))}
              </div>}
        ...
        <div className="panel p-4 text-sm">
          <div className="flex gap-1 text-xs mb-3 bg-slate-100 border border-slate-200 rounded-lg p-1" role="tablist">{(['info', 'evidence', 'history', 'qr', 'report'] as const).map(t =>
            <button key={t} role="tab" aria-selected={tab === t} onClick={() => setTab(t)} className={`flex-1 px-2 py-1.5 rounded-md capitalize font-medium transition-colors ${tab === t ? 'bg-gov-navy text-white' : 'text-slate-400 hover:text-slate-800'}`}>{t}</button>)}</div>
          ...
          {unit && tab === 'info' && <>
            ...
            <div className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-1.5 mt-2">Prototype 3D ULPIN: <b className="font-mono">{unit.prototype_ulpin}</b><br />Internal ID: <span className="font-mono">{unit.internal_id}</span><br /><span className="text-amber-400/80">Prototype reference — NOT an official Govt. of India ULPIN.</span></div>
            <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-xs mt-2.5 text-slate-600">
              <div>Parcel: <span className="text-slate-800">{unit.parcel}</span></div><div>Floor: <span className="text-slate-800">{unit.floor_label} ({unit.floor})</span></div>
              <div>Area: <span className="text-slate-800">{unit.area_sqft} sq.ft</span></div><div>Vertical: <span className="text-slate-800">{unit.z_min}–{unit.z_max}m</span></div>
              <div>Type: <span className="text-slate-800">{unit.unit_type}</span></div><div>Owner: <span className="text-slate-800">{unit.owner?.display_name || '— missing'}</span></div>
            </div>
            ...
            <Link to={'/property/' + unit.prototype_ulpin} className="text-xs text-gov-navy underline underline-offset-2">Open public digital identity →</Link>
          </>}
          ...
          {unit && tab === 'report' && <div className="text-xs space-y-1 text-slate-600">
            <div className="font-semibold text-slate-900">Property Report (in-browser)</div>
