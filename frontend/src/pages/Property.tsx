...
export default function Property() {
  ...
  if (isLoading) return <PublicLayout><div className="panel-pad text-sm text-slate-500">{t('prop.loading')}</div></PublicLayout>;
  if (isError || !u) return (
    <PublicLayout>
      <div className="max-w-lg mx-auto panel-pad text-sm text-center">
        <div className="font-bold text-lg text-slate-900">{t('prop.notfound')}</div>
        <div className="text-slate-500 mt-1 font-mono text-xs">{ulpin}</div>
        <div className="text-slate-500 mt-1">{t('prop.check')}</div>
        <div className="flex gap-2 justify-center mt-4">
          <Link to="/verify" className="btn-primary">{t('prop.verifyAnother')}</Link>
          <Link to="/" className="btn-ghost">{t('prop.portalHome')}</Link>
        </div>
      </div>
    </PublicLayout>
  );
  ...
  return (
    <PublicLayout trail={[{ label: t('trail.home'), to: '/' }, { label: t('trail.verify'), to: '/verify' }, { label: u.prototype_ulpin }]}>
      <div className="max-w-lg mx-auto panel-pad text-sm space-y-3.5">
        <div className="flex items-center gap-2">
          <ShieldCheck size={20} className="text-gov-green" />
          <div className="font-bold text-lg text-slate-900">{t('prop.title')}</div>
        </div>
        <div className="text-[11px] text-amber-800 bg-amber-50 border border-amber-200 rounded-lg p-2.5">{t('c.protoNotice')}</div>
        ...
        <div className="grid grid-cols-2 gap-1.5 text-xs text-slate-500">
          {[[t('g.parcel'), u.parcel], [t('g.building'), u.building], [t('g.floor'), u.floor_label], [t('g.unit'), u.unit],
            [t('g.area'), u.area_sqft + ' sq.ft'], [t('g.vertical'), `${u.z_min ?? '—'}–${u.z_max ?? '—'}m`]].map(([k, v]) =>
        ...
        <div className="text-[11px] text-slate-500">{t('prop.foot')}</div>
      </div>
    </PublicLayout>
  );
}
