...
import { TOUR_STEPS, COPILOT_DEMO_Q } from '../lib/tourSteps';
...
export default function DemoTour({ onCopilot }: { onCopilot: () => void }) {
  ...
  useEffect(() => {
    ...
    if (s.action === 'copilot-ask' && asked.current !== tour) {
      ...
      api.ai(COPILOT_DEMO_Q)
        .then(r => { setHighlights(r.highlights || []); setToast(`Copilot: ${r.count} height mismatches highlighted`); })
        .catch((e: any) => setToast('Copilot unavailable: ' + e.message));
    }
  }, [tour]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      document.querySelectorAll('.tour-spot').forEach(e => e.classList.remove('tour-spot'));
      const el = document.querySelector(`[data-tour="${TOUR_STEPS[tour].anchor}"]`);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setTour(tour >= TOUR_STEPS.length - 1 ? null : tour + 1), 9000);
    return () => clearTimeout(t);
  }, [tour, auto]);
      ...
      <div className="panel p-4 !border-gov-saffron shadow-panel">
        ...
        <div className="flex gap-2 mt-3">
          <button disabled={tour === 0} onClick={() => setTour(tour - 1)}
            className="btn-ghost !text-xs disabled:opacity-40 flex items-center gap-1"><ChevronLeft size={13} />Back</button>
