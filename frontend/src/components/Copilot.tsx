  ...
  return (
    <div data-tour="tour-copilot" className="fixed bottom-4 right-4 w-[400px] max-w-[92vw] h-[560px] max-h-[80vh] bg-white border border-slate-200 rounded-2xl shadow-panel flex flex-col z-50 overflow-hidden">
      <div className="flex items-center gap-2.5 px-4 py-3 border-b border-slate-200 bg-slate-50">
        ...
        <div><div className="font-bold text-sm text-slate-900">Cadastral Copilot</div>
        <div className="text-[11px] text-slate-500">AI-assisted · requires official verification</div></div>
        <button className="ml-auto text-slate-400 hover:text-slate-900 p-1" onClick={onClose} aria-label="Close copilot"><X size={16} /></button>
      </div>
      <div ref={logRef} className="flex-1 overflow-y-auto p-3 space-y-2.5 scrollthin text-sm bg-white">
        {log.length === 0 && (
          <div className="text-center pt-6">
            ...
            <div className="text-slate-600 text-xs font-medium">Ask about parcels, floors, conflicts, utilities…</div>
            <div className="text-slate-400 text-[11px] mt-1">Answers highlight matching objects on the map.</div>
          </div>)}
        {!!highlights.length && <button onClick={() => setHighlights([])} className="text-[11px] text-gov-navy underline underline-offset-2">Clear {highlights.length} map highlight{highlights.length === 1 ? '' : 's'}</button>}
