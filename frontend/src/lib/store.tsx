...
const Ctx = createContext<{ auth: Auth; setAuth: (a: Auth) => void; highlights: any[]; setHighlights: (h: any[]) => void;
  ...
  tour: number | null; setTour: (t: number | null) => void }>(
...
export const Store = ({ children }: { children: React.ReactNode }) => {
  ...
  return <Ctx.Provider value={{ auth, setAuth, highlights, setHighlights, toast, setToast, tour, setTour }}>{children}</Ctx.Provider>;
};
